...
use crate::{LanesAtMost64, SimdI128, SimdI16, SimdI32, SimdI64, SimdI8, SimdIsize};
...
macro_rules! define_opaque_mask {
    {
        ...
        $(#[$attr])*
        #[allow(non_camel_case_types)]
        pub struct $name<const LANES: usize>($inner_ty<LANES>) where $bits_ty<LANES>: LanesAtMost64;

        impl_opaque_mask_reductions! { $name, $inner_ty, $bits_ty }

        impl<const LANES: usize> $name<LANES>
        where
            $bits_ty<LANES>: LanesAtMost64
        {
        ...
        where
            $bits_ty<LANES>: crate::LanesAtMost64
        {
